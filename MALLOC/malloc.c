...
// 在适当的位置添加这个函数定义
u16 mem_perused(u8 memx)
{
    extern __align(4) u8 mem1base[MEM1_MAX_SIZE]; 
    extern __align(4) u8 mem2base[MEM2_MAX_SIZE] __attribute__((at(0X68000000)));
    extern u16 mem1mapbase[MEM1_ALLOC_TABLE_SIZE];
    extern u16 mem2mapbase[MEM2_ALLOC_TABLE_SIZE];
    ...
    if(memx == SRAMIN) {
        ...
        return (used*MEM1_BLOCK_SIZE*100)/MEM1_MAX_SIZE;
    } else if(memx == SRAMEX) {
        for(i=0; i<MEM2_ALLOC_TABLE_SIZE; i++) {
            if(mem2mapbase[i]) used++;
        }
        return (used*MEM2_BLOCK_SIZE*100)/MEM2_MAX_SIZE;
    }
    return 0;
}
